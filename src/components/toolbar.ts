...
export class AppToolbar extends LitElement {
  ...
  render() {
    ...
    return html`
            <div class="toolbar">
                <button class="tool-btn ${this.activeTool === 'selection' ? 'active' : ''}" @click=${() => this.setTool('selection')}>Selection Tool</button>
                <button class="tool-btn ${this.activeTool === 'component' ? 'active' : ''}" @click=${() => this.setTool('component')}>
                    Component Tool
                    <div class="preview-box">${def ? html`<img src=${def.img.src} />` : html`<span>...</span>`}</div>
                </button>
                <button class="tool-btn ${this.activeTool === 'wire' ? 'active' : ''}" @click=${() => this.setTool('wire')}>Wiring Tool</button>
            </div>
        `;
  }
  ...
  static styles = css`
        ...
        .tool-btn.active { background: #ddd; font-weight: bold; }
